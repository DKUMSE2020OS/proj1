...
void add_queue(struct Run_q* run_q,int pid,int burst, int wait, int again);
...
int second_exec_time = 7;
int pids[10];
int time_quantum[5];
int total_CPU_burst_time=0;
int second_CPU_burst_time=0;
...
int main()
{
	...
	for (int i = 0 ; i < 10; i++) {	
		...
		if (ret < 0) {
			...
			//InitQueue(run_q);
			add_queue(&run_q,pids[i],burst, 0, 0);
...
//			wait = rand()%10+1;
	                msg.io_time = wait;
//			second_exec_time = rand()%10+1;
			msg.next_exec_time = second_exec_time;
        	        ret = msgsnd(msgq, &msg, sizeof(msg), 0);
	                printf("msgsnd ret: %d\n", ret);
//	                wait = msg.io_time;
//			second_exec_time = msg.next_exec_time;
	//		wait = 5;
//			second_exec_time = 7;
			printf("@@@@@@@@@@@@@@@@@:%d\n", second_exec_time);
		}
...
//			w_PCB->burst_time = 7;
			add_queue(&run_q, w_PCB->pid, w_PCB->burst_time, w_PCB->remaining_wait, 1);
		}
...
//                 	int msgq;
			...
			r_PCB->burst_time = msg.next_exec_time;
			second_CPU_burst_time += r_PCB->burst_time;
			r_PCB = pop_queue(&run_q);
                        add_queue(&wait_q,r_PCB->pid, r_PCB->burst_time, msg.io_time, 0);
//                      r_PCB->remaining_wait = 5;
//			r_PCB->remaining_wait = msg.io_time;
		}
		else if(r_PCB->time_quantum == 0){
			r_PCB = pop_queue(&run_q);
			add_queue(&run_q,r_PCB->pid, r_PCB->burst_time, 0, r_PCB->again);
		}
		else{
...
//		printf("   H1: finished!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");	
//	}
	if ((count == total_CPU_burst_time+second_CPU_burst_time) & (IsEmpty(&run_q)) & (IsEmpty(&wait_q))){
		printf("******************************REALLY BYE~~: %d\n", second_CPU_burst_time);
