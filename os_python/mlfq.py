...
shared_c3_turn = multiprocessing.Value('d',0)
...
def arrange(a,b):
    if shared_c_turn.value == 1:
        ...
        third_ready_q[0].cpu_burst = shared_cpu_burst.value
        third_ready_q[0].c_time_q = shared_c_time_q.value
        shared_c3_turn.value = 0
        third_ready_q[0].c_turn = shared_c3_turn.value
    ...
    if sec_ready_q != []:
        if sec_ready_q[0].cpu_burst==0:
            ...
            tmp = sec_ready_q.pop(0)
            tmp.c_time_q = 4
            third_ready_q.append(tmp)

    if third_ready_q != []:
        if third_ready_q[0].cpu_burst==0:
            tmp = third_ready_q.pop(0)
            tmp.c_time_q = 3
            wait_q.append(tmp)
        elif third_ready_q[0].c_time_q == 0:
            tmp = third_ready_q.pop(0)
            tmp.c_time_q = 5
            third_ready_q.append(tmp)
        
        for x in range(len(third_ready_q)):
            if third_ready_q[x].cpu_burst<=3:
                tmp = third_ready_q.pop(x)
                tmp.c_time_q = 2
                ready_q.append(tmp)
                break
...
def parent():
    ...
    if third_ready_q !=[] and third_ready_q[0].c_turn == 0:
        third_ready_q[0].c_turn = 1
        shared_cpu_burst.value = third_ready_q[0].cpu_burst
        shared_c_time_q.value = third_ready_q[0].c_time_q
        shared_c3_turn.value = third_ready_q[0].c_turn
        os.kill(ready_q[0].pid,signal.SIGUSR1)
        while end.value == 0:
            tr =0
        end.value =0
...
def ready_init(pid):
    new_pcb = PCB()
    new_pcb.pid = pid
    new_pcb.cpu_burst = random.randint(1,15)
...
def display():
    ...
    print('******************third_ready_q****************************')
    for x in range(len(third_ready_q)):
        print('id : ',third_ready_q[x].pid,'cpu :',third_ready_q[x].cpu_burst ,'io : ',third_ready_q[x].io_burst,'time_q : ',third_ready_q[x].c_time_q)
